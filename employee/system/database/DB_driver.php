...
abstract class CI_DB_driver {
		...
		protected function _get_operator($str)
	{
		...
		if (empty($_operators))
		{
			$_les = ($this->_like_escape_str !== '')
				...
				: '';
			$_operators = array(
				'\s*(?:<|>|!)?=\s*',             				'\s*<>?\s*',                     				'\s*>\s*',                       				'\s+IS NULL',                    				'\s+IS NOT NULL',                				'\s+EXISTS\s*\(.*\)',        				'\s+NOT EXISTS\s*\(.*\)',    				'\s+BETWEEN\s+',                 				'\s+NOT BETWEEN\s+',             				'\s+IN\s*\(.*\)',            				'\s+NOT IN\s*\(.*\)',        				'\s+LIKE\s+\S.*('.$_les.')?',    				'\s+NOT LIKE\s+\S.*('.$_les.')?' 			);
		...
		protected function _cache_init()
	{
		...
		$this->CACHE = new CI_DB_Cache($this); 		return TRUE;
	}
