...
class CI_Zip {
		...
		protected function _add_dir($dir, $file_mtime, $file_mdate)
	{
		...
		$this->zipdata .=
			...
			.pack('v', $file_mtime)
			.pack('v', $file_mdate)
			.pack('V', 0) 			.pack('V', 0) 			.pack('V', 0) 			.pack('v', self::strlen($dir)) 			.pack('v', 0) 			.$dir
						.pack('V', 0) 			.pack('V', 0) 			.pack('V', 0); 
		$this->directory .=
			"\x50\x4b\x01\x02\x00\x00\x0a\x00\x00\x00\x00\x00"
			.pack('v', $file_mtime)
			.pack('v', $file_mdate)
			.pack('V',0) 			.pack('V',0) 			.pack('V',0) 			.pack('v', self::strlen($dir)) 			.pack('v', 0) 			.pack('v', 0) 			.pack('v', 0) 			.pack('v', 0) 			.pack('V', 16) 			.pack('V', $this->offset) 			.$dir;
		...
		protected function _add_data($filepath, $data, $file_mtime, $file_mdate)
	{
		...
		$this->zipdata .=
			...
			.pack('v', self::strlen($filepath)) 			.pack('v', 0) 			.$filepath
			.$gzdata; 
		$this->directory .=
			...
			.pack('V', $compressed_size)
			.pack('V', $uncompressed_size)
			.pack('v', self::strlen($filepath)) 			.pack('v', 0) 			.pack('v', 0) 			.pack('v', 0) 			.pack('v', 0) 			.pack('V', 32) 			.pack('V', $this->offset) 			.$filepath;
		...
		public function get_zip()
	{
				if ($this->entries === 0)
		{
			...
			.pack('v', $this->entries) 			.pack('v', $this->entries) 			.pack('V', self::strlen($this->directory)) 			.pack('V', self::strlen($this->zipdata)) 			."\x00\x00"; 		return $this->zipdata.$footer;
	}
