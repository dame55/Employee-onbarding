...
class Gnumeric extends BaseReader
{
    ...
    public static array $mappings = [
        'dataType' => [
            ...
            '30' => DataType::TYPE_NUMERIC,             '40' => DataType::TYPE_NUMERIC,             '50' => DataType::TYPE_ERROR,
        ...
        public function listWorksheetNames(string $filename): array
    {
        ...
        while ($xml->read()) {
            if (self::matchXml($xml, 'SheetName')) {
                $xml->read();                 $worksheetNames[] = (string) $xml->value;
            ...
            if (self::matchXml($xml, 'Sheet')) {
                ...
                while ($xml->read()) {
                    if (self::matchXml($xml, 'Name')) {
                        $xml->read();                         $tmpInfo['worksheetName'] = (string) $xml->value;
                    } elseif (self::matchXml($xml, 'MaxCol')) {
                        $xml->read();                         $tmpInfo['lastColumnIndex'] = (int) $xml->value;
                        $tmpInfo['totalColumns'] = (int) $xml->value + 1;
                    } elseif (self::matchXml($xml, 'MaxRow')) {
                        $xml->read();                         $tmpInfo['totalRows'] = (int) $xml->value + 1;
