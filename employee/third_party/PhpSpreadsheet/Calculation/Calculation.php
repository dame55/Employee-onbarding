...
class Calculation
{
        ...
        if (count(self::$validLocaleLanguages) == 1) {
            ...
            self::$localeBoolean = ['TRUE' => 'TRUE', 'FALSE' => 'FALSE', 'NULL' => 'NULL'];
                ...
                foreach ($localeFunctions as $localeFunction) {
                    [$localeFunction] = explode('##', $localeFunction);                     if (str_contains($localeFunction, '=')) {
                ...
                foreach ($localeSettings as $localeSetting) {
                    [$localeSetting] = explode('##', $localeSetting);                     if (str_contains($localeSetting, '=')) {
        ...
        private function convertMatrixReferences(string $formula): false|string
    {
        ...
        ':' => 9,         '∩' => 8,         '∪' => 7,         '~' => 6,         '%' => 5,         '^' => 4,         '*' => 3, '/' => 3,         '+' => 2, '-' => 2,         '&' => 1,         '>' => 0, '<' => 0, '=' => 0, '>=' => 0, '<=' => 0, '<>' => 0,     ];
        ...
        private function internalParseFormula(string $formula, ?Cell $cell = null): bool|array
    {
        ...
        $output = [];
        $expectingOperator = false;                 $expectingOperand = false;         
                        while (true) {
            ...
            if ($opCharacter === '-' && !$expectingOperator) {                                                $stack->push('Unary Operator', '~');
                ++$index;             } elseif ($opCharacter === '%' && $expectingOperator) {
                                $stack->push('Unary Operator', '%');
                ++$index;
            } elseif ($opCharacter === '+' && !$expectingOperator) {                            ++$index;             } elseif ((($opCharacter === '~') || ($opCharacter === '∩') || ($opCharacter === '∪')) && (!$isOperandOrFunction)) {
                                return $this->raiseFormulaError("Formula Error: Illegal character '~'");             } elseif ((isset(self::CALCULATION_OPERATORS[$opCharacter]) || $isOperandOrFunction) && $expectingOperator) {                    while (
                    $stack->count() > 0
                    && ($o2 = $stack->last())
            ...
            } elseif ($opCharacter === ')' && $expectingOperator) {                 $expectingOperand = false;
                while (($o2 = $stack->pop()) && $o2['value'] !== '(') {                     $output[] = $o2;
                }
                $d = $stack->last(2);
                ...
                if (is_array($d) && preg_match('/^' . self::CALCULATION_REGEXP_FUNCTION . '$/miu', $d['value'], $matches)) {
                    ...
                    $functionName = $matches[1];                     $d = $stack->pop();
                    $argumentCount = $d['value'] ?? 0;                     $output[] = $d;                     $output[] = $stack->pop();                     if (isset(self::$controlFunctions[$functionName])) {
                        $expectedArgumentCount = self::$controlFunctions[$functionName]['argumentCount'];
                    } elseif (isset(self::$phpSpreadsheetFunctions[$functionName])) {
                        $expectedArgumentCount = self::$phpSpreadsheetFunctions[$functionName]['argumentCount'];
                    } else {                            return $this->raiseFormulaError('Formula Error: Internal error, non-function on stack');
                    }
                ...
                if (preg_match('/^' . self::CALCULATION_REGEXP_FUNCTION . '$/miu', $val, $matches)) {
                    ...
                    if (isset(self::$phpSpreadsheetFunctions[strtoupper($matches[1])]) || isset(self::$controlFunctions[strtoupper($matches[1])])) {                            $valToUpper = strtoupper($val);
                    } else {
                    ...
                    if ($testPrevOp !== null && $testPrevOp['value'] === ':') {
                        ...
                        if (
                            ...
                            if (ctype_digit($val) && $val <= 1048576) {
                                                                ...
                                                                $valx = $val;
                                $endRowColRef = ($refSheet !== null) ? $refSheet->getHighestDataColumn($valx) : AddressRange::MAX_COLUMN;                                 $val = "{$rangeWS2}{$endRowColRef}{$val}";
                            } elseif (ctype_alpha($val) && strlen($val ?? '') <= 3) {
                                                                $stackItemType = 'Column Reference';
                                $endRowColRef = ($refSheet !== null) ? $refSheet->getHighestDataRow($val) : AddressRange::MAX_ROW;                                 $val = "{$rangeWS2}{$val}{$endRowColRef}";
                            }
                            $stackItemReference = $val;
                        }
                ...
                $index += $length;
            } elseif ($opCharacter === '$') {                 ++$index;
            } elseif ($opCharacter === ')') {                 if ($expectingOperand) {
                    $output[] = ['type' => 'Empty Argument', 'value' => self::$excelConstants['NULL'], 'reference' => 'NULL'];
                    $expectingOperand = false;
                    $expectingOperator = true;
                } else {
            ...
            } else {                    return $this->raiseFormulaError('Formula Error: An unexpected error occurred');
            }
            ...
            if ($formula[$index] === ' ') {
                ...
                if (
                    ...
                    while (
                        ...
                        $output[] = $stack->pop();                     }
                    $stack->push('Binary Operator', '∩');                     $expectingOperator = false;
                }
            }
        }
            ...
            if ($token instanceof Operands\StructuredReference) {
                ...
                $operand2 = self::dataTestReference($operand2Data);
                    ...
                    $this->debugLog->writeDebugLog('Evaluating %s %s %s', $this->showValue($operand1), $token, $this->showValue($operand2));
                                ...
                                switch ($token) {
                                        case '>':                     case '<':                     case '>=':                     case '<=':                     case '=':                     case '<>':                         $result = $this->executeBinaryComparisonOperation($operand1, $operand2, (string) $token, $stack);
                        if (isset($storeKey)) {
                            $branchStore[$storeKey] = $result;
                        }

                        break;
                                        case ':':                         if ($operand1Data['type'] === 'Defined Name') {
                        ...
                        break;
                    case '+':                                case '-':                                case '*':                                case '/':                                case '^':                                    $result = $this->executeNumericBinaryOperation($operand1, $operand2, $token, $stack);
                        if (isset($storeKey)) {
                            $branchStore[$storeKey] = $result;
                        }

                        break;
                    case '&':                                                                                                            $operand1 = self::boolToString($operand1);
                        ...
                        break;
                    case '∩':                                    $rowIntersect = array_intersect_key($operand1, $operand2);
                ...
                if ((isset(self::$phpSpreadsheetFunctions[$functionName])) || (isset(self::$controlFunctions[$functionName]))) {                        $passByReference = false;
