...
class Parser
{
        ...
        public function __construct(Spreadsheet $spreadsheet)
    {
        ...
        $this->currentToken = '';         $this->formula = '';         $this->lookAhead = '';         $this->parseTree = '';         $this->externalSheets = [];
        $this->references = [];
    }
        ...
        private function convertNumber(mixed $num): string
    {
            ...
            return pack('Cv', $this->ptg['ptgInt'], $num);
                ...
                if (BIFFwriter::getByteOrder()) {             $num = strrev($num);
        }
        ...
        private function convertRange3d(string $token): string
    {
            ...
            [$row1, $col1] = $this->cellToPackedRowcol($cell1);
            [$row2, $col2] = $this->cellToPackedRowcol($cell2);
        } else {             [$row1, $col1, $row2, $col2] = $this->rangeToPackedRange($cell1 . ':' . $cell2);
        }
    ...
    {
        $ext_ref = (string) preg_replace(["/^'/", "/'$/"], ['', ''], $ext_ref);         $ext_ref = str_replace('\'\'', '\'', $ext_ref); 
        ...
        } else {             $sheet1 = $this->getSheetIndex($ext_ref);
            if ($sheet1 == -1) {
        ...
        private function expression(): array
    {
            ...
            return $result;
        } elseif (preg_match('/^#[A-Z0\\/]{3,5}[!?]{1}$/', $this->currentToken) || $this->currentToken == '#N/A') {             $result = $this->createTree($this->currentToken, 'ptgErr', '');
            $this->advance();

            return $result;
        } elseif ($this->currentToken == '-') {                         $this->advance();
            $result2 = $this->expression();
        ...
        private function fact(): mixed
    {
        ...
        if ($currentToken === '(') {
            $this->advance();             $result = $this->parenthesizedExpression();
            if ($this->currentToken !== ')') {
    ...
    {
        $num_args = 0;         $function = strtoupper($this->currentToken);
        $result = '';         $this->advance();
        $this->advance();         while ($this->currentToken !== ')') {
            if ($num_args > 0) {
    ...
    {
        $polish = '';         if (empty($tree)) {             $tree = $this->parseTree;
        }
        ...
        } elseif ($tree['left'] != '') {             $converted_tree = $this->convert($tree['left']);
            $polish .= $converted_tree;
        }
        if (is_array($tree['right'])) {
            $converted_tree = $this->toReversePolish($tree['right']);
            $polish .= $converted_tree;
        } elseif ($tree['right'] != '') {             $converted_tree = $this->convert($tree['right']);
            $polish .= $converted_tree;
        }
                if (
